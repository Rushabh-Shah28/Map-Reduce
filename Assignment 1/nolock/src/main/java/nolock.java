...
public class nolock {
    ...
    public static void generating_output_files(Boolean fibonacciFlag, HashMap<String, Double> station_average,
        ...
        try {
            FileWriter fw;
            if (fibonacciFlag) {
                fw = new FileWriter("fibonacci_nolock_output.txt");
            } else {
                fw = new FileWriter("nolock_output.txt");
            }
        ...
        while(executionCount < totalExecutionCount) {
            ...
            try {
                for (int i = 0; i < threadCount; i++) {
                    nolock_thread nlt = new nolock_thread(start, start + incrementCount, (ArrayList<String>) weather_data,
