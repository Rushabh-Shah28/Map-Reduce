...
public class fineLock {
    ...
    public static void generating_output_files(Boolean fibonacciFlag, HashMap<String, Double> station_average,
        ...
        try {
            FileWriter fw;
            if (fibonacciFlag) {
                fw = new FileWriter("fibonacci_FineLock_output.txt");
            } else {
                fw = new FileWriter("FineLock_output.txt");
            }
            ...
            while(executionCount < totalExecutionCount) {
                ...
                try {
                    for (int i = 0; i < threadCount; i++) {
                        fineLock_thread flt = new fineLock_thread(start, start + incrementCount,
                                (ArrayList<String>) weather_data, station_temperature, station_count, fibonacciFlag);
                        Thread t = new Thread(flt);
                        threadArray.add(t);
